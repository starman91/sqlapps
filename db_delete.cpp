...
///
//////////////////////////////////////////////////////////////////////////////////////////
bool DBDelete::removeFromDB (QSqlDatabase& targetDB, const QString tableName, const QStringList qualNames, const QStringList qualVals)
{
	...
	//Prep that statement
	queryString = "DELETE FROM " + tableName + " WHERE " + qualifierString + ";";
	#ifdef DEBUG
//		std::cout << queryString.toAscii().constData() << "\n";
	...
	if (!success)
	{
		...
		std::cerr << "ERROR: Postgres delete preparation failed for: " << queryString.toAscii().constData() << "\n";
	...
	{
		QSqlError error = query->lastError();
		std::cerr << "ERROR: Postgres delete failed: " << queryString.toAscii().constData() << "\n";
		std::cerr << "Error message: " << error.text().toAscii().constData() << "\n";
		return false;
	}
	return true;
